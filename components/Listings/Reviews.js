...
export default function Reviews({
  ...
  if (!listingRating || numberOfReviews === 0) {
    return <div className="text-2xl">No Reviews (yet)</div>;
  }
      ...
      <div>
        ...
        <div className="flex gap-2 items-center leading-loose ">
          ...
          <span className="text-2xl">
            {listingRating
              ? (
                  Math.round((listingRating / 20 + Number.EPSILON) * 100) / 100
                ).toFixed(2)
              : "New"}
          </span>
          <span className="text-2xl">·</span>
          <span className="text-2xl">{numberOfReviews + " reviews"}</span>
        </div>
        {/* Score graph */}
        <div className="pt-8 hidden md:grid grid-cols-2 grid-rows-3 gap-4">
          <div className="mr-12 flex justify-between items-center">
            ...
            <div className="my-auto w-1/2 h-1  bg-slate-200 rounded-full">
              <div
                style={{
                  width: ((cleanlinessRating / 2).toFixed(2) / 5) * 100 + "%",
                }}
          ...
          <div className="mr-12 flex justify-between items-center">
            ...
            <span className="my-auto w-1/2 h-1  bg-slate-200 rounded-full">
              <div
                style={{
                  width: ((accuracyRating / 2).toFixed(2) / 5) * 100 + "%",
                }}
                className="h-1 bg-black rounded-full"
              ></div>
            </span>
          ...
          <div className="mr-12 flex justify-between items-center">
            ...
            <span className="my-auto w-1/2 h-1  bg-slate-200 rounded-full">
              <div
                style={{
                  width: ((communicationRating / 2).toFixed(2) / 5) * 100 + "%",
                }}
                className="h-1 bg-black rounded-full"
              ></div>
            </span>
            <span className="mx-2 text-xs font-bold ">
              {communicationRating
                ? (communicationRating / 2).toFixed(2)
                : "New"}
            </span>
          </div>
          <div className="mr-12 flex justify-between items-center">
            ...
            <span className="my-auto w-1/2 h-1  bg-slate-200 rounded-full">
              <div
                style={{
                  width: ((locationRating / 2).toFixed(2) / 5) * 100 + "%",
                }}
                className="h-1 bg-black rounded-full"
              ></div>
            </span>
          ...
          <div className="mr-12 flex justify-between items-center ">
            ...
            <span className="my-auto w-1/2 h-1  bg-slate-200 rounded-full">
              <div
                style={{
                  width: ((checkinRating / 2).toFixed(2) / 5) * 100 + "%",
                }}
                className="h-1 bg-black rounded-full"
              ></div>
            </span>
          ...
          <div className="mr-12 flex justify-between items-center">
            ...
            <span className="my-auto w-1/2 h-1  bg-slate-200 rounded-full">
              <div
                style={{
                  width: ((valueRating / 2).toFixed(2) / 5) * 100 + "%",
                }}
                className="h-1 bg-black rounded-full"
              ></div>
            </span>
        ...
        return (
          <div key={"review" + i} className="mr-16">
            ...
            <p className="mt-2 line-clamp-3">
              {lorem.substring(0, Math.floor(Math.random() * lorem.length))}
            </p>
          </div>
        );
      })}
